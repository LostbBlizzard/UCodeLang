...
void SystematicAnalysis::OnClassNode(const ClassNode& Node)
{
	...
	if (passtype == PassType::FixedTypes)//auto make funcs
	{
		...
		if (ClassInf->_WillHaveFielddeInit && GetSymbol(_Table._Scope.GetApendedString((String)ClassDestructorFunc),SymbolType::Func) == nullptr)
		{
			auto DropFunc = &AddSybol(SymbolType::Func, ClassDestructorFunc,_Table._Scope.GetApendedString((String)ClassDestructorFunc));
	...
	if (passtype == PassType::BuidCode)
	{
		...
		{//de-init func
			...
			LookingAtIRBlock = LookingAtIRFunc->NewBlock(".");
			{
				for (int i = ClassInf->Fields.size() - 1; i >= 0; i--)
				{
